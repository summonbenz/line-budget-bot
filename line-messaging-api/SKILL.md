...
description: Send messages, look up users, configure rich menus, set up webhooks, and pull analytics for LINE Official Accounts and bots via direct curl calls to the LINE Messaging API. Use this skill whenever the user mentions a LINE bot, a LINE Official Account, the LINE Messaging API, sending LINE push/reply/broadcast/multicast messages, LINE rich menus, LINE webhook setup or testing, LINE follower lists or user profile lookups, or LINE bot analytics/delivery stats — even if they just say things like "send a message to my LINE bot's followers," "set up a rich menu," or "check how many people are following my LINE account" without naming the API explicitly.
---
...
After setting the endpoint, verify it actually works with **Test Webhook** below — LINE will attempt a real connection to the URL and report back whether it succeeded.
...
The response returns a `richMenuId` — you need it for the next two steps. A rich menu is not visible to anyone until you've uploaded its image AND set it as default (or linked it to specific users).

---

### 16. Upload Rich Menu Image

Attach the image that gives the rich menu its visual buttons. This uses a different host (`api-data.line.me`, not `api.line.me`) and sends raw binary, not JSON. The image must be JPEG or PNG, under 1MB, and match the `size` declared when the rich menu was created (e.g. 2500x1686).

```bash
curl -s -X POST "https://api-data.line.me/v2/bot/richmenu/RICH_MENU_ID/content" --header "Content-Type: image/png" --header "Authorization: Bearer $LINE_TOKEN" --data-binary @/path/to/image.png | jq .
```

---

### 17. Set Default Rich Menu

Make a rich menu show up automatically for every user who doesn't have one individually linked. Without this step (or per-user linking), a created and image-uploaded rich menu still won't appear in anyone's chat.

```bash
curl -s -X POST "https://api.line.me/v2/bot/user/all/richmenu/RICH_MENU_ID" --header "Authorization: Bearer $LINE_TOKEN" | jq .
```
...
### 19. Get Message Delivery Statistics
...
4. **Rich menu is a 3-step workflow, not 1**: create (step 15) → upload image (step 16) → set as default or link to users (step 17). Skipping either of the last two leaves the menu invisible to everyone even though the API call in step 15 "succeeded"
